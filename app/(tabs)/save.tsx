...
const Save = () => {
    ...
    // For complex layouts like masonry, we still need some StyleSheet for dynamic sizing
    // Fixed getMasonryImageStyle function
    const getMasonryImageStyle = (imgIndex: number) => {
        switch (imgIndex % 3) {
            case 0: return { width: '48%', height: 350 }; // First image - full height
            case 1: return { width: '48%', height: 167.5 }; // Second image - half height (with gap consideration)
            case 2: return { width: '48%', height: 167.5 }; // Third image - half height (with gap consideration)
            default: return { width: '48%', height: 167.5 };
        }
    };

    // Fixed renderCollection function
    const renderCollection = ({ item, index }: { item: Collections; index: number }) => {
        return (
            <View className="bg-white rounded-2xl  mb-5 p-4 shadow-sm w-full">
                ...
                <View className="flex-row justify-between">
                    ...
                    <View style={{ width: '48%' }}>
                        {item.images[0] && (
                            <TouchableOpacity className="relative mb-3 rounded-xl overflow-hidden shadow-sm">
                                <Image
                    ...
                    <TouchableOpacity className="absolute bottom-6 right-2 bg-white rounded-full p-2 shadow-sm">
                        <Bookmark
                            size={20}
                            color={colors.black}
                            fill={colors.light}
    ...
    return (
        <SafeAreaView className="flex-1 bg-gray-50">
            <ScrollView className="flex-1 bg-gray-50">
                {/* Header */}
                <View className="px-2 py-1 ml-2">
                    <Text className="text-2xl font-bold text-black">Save</Text>
                </View>

                {/* Tab Navigation */}
                <View className="flex-row px-2 mb-5">
               ...
               {(activeTab === "Collections" || activeTab === "Items") && <View className="flex-row items-center pl-6 mb-4">
                ...
                {/* Filter Panel */}
                {showFilters && (
                    <View className="bg-white py-4 border-t border-b border-gray-200">
                ...
                {/* Items Grid */}
                {(activeTab === "Collections" || activeTab === "Outfits") && (
                    <FlatList
                        data={collection}
